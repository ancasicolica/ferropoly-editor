...
router.get('/load/:gameId', async function (req, res) {
  try {
    const gameplayData = await gameplays.getGameplay(
      req.params.gameId,
    ...
    if (
      _.get(gameplayData, 'internal.owner', 'none') !==
      req.session.passport.user
    ) {
      return res.status(401).send({ message: 'Zugriff nicht erlaubt' });
    }
    // Now get all properties of this gameplay
    const propertyData = await properties.getPropertiesForGameplay(
      req.params.gameId,
      { lean: true }
    );
    ...
    if (!propertyData) {
      return res
        .status(500)
        .send({ message: 'Spielfeld konnte nicht geladen werden' });
    }
    ...
    return res
      .status(500)
      .send({ message: 'Fehler beim Laden der Orte: ' + err.message });
  }
});
...
router.post('/save/:gameId', async function (req, res) {
  if (!req.body.authToken || req.body.authToken !== req.session.authToken) {
    logger.info(
      `/save/${req.params.gameId} : Auth token missing or wrong, access denied: ${req.body.authToken} vs ${req.session.authToken}`
    );
    logger.info('Passport Info:' + _.get(req, 'session.passport', 'none'));
    return res
      .status(401)
      .send({ message: 'Kein Zugriff möglich, bitte einloggen' });
  }
  try {
    if (req.params.gameId !== req.body.gameplay.internal.gameId) {
      logger.info(
        `/save/${req.params.gameId} : GameId mismatch: ${req.params.gameId} vs ${req.body.gameplay.internal.gameId}`
      );
    ...
    await gameplays.updateGameplayPartial(req.body.gameplay);
    return res.send({
      success: true,
      gameId: req.body.gameplay.internal.gameId,
    });
  } catch (err) {
    logger.error('updateGameplay failed', err);
    return res
      .status(500)
      .send({ message: 'Fehler beim Update des Spieles: ' + err.message });
  }
});
...
router.post('/registration/:gameId', (req, res) => {
  if (!req.body.authToken || req.body.authToken !== req.session.authToken) {
    logger.info(
      `/registration/${req.params.gameId} : Auth token missing or wrong, access denied: ${req.body.authToken} vs ${req.session.authToken}`
    );
    logger.info('Passport Info:' + _.get(req, 'session.passport', 'none'));
    return res
      .status(401)
  ...
  gameplays
    .updateRegistrationData(req.params.gameId, req.body.gameplay)
    .then((info) => {
...
router.post('/saveProperty/:gameId', async function (req, res) {
  try {
    ...
    if (_.get(prop, 'pricelist.priceRange', -1) < 0) {
      ...
      prop.pricelist = _.omit(prop.pricelist, [
        'rents',
        'propertyGroup',
        'pricePerHouse',
        'price',
      ...
      logger.info(
        'Removed Property from previous price list: ' + prop.location.name
      );
    }

    logger.info('Save property ' + prop.location.name);
    const updatedProp = await properties.updateProperty(
      req.params.gameId,
      prop
    );

    // Updating a property invalidates the pricelist!
    await gameplays.invalidatePricelist(
      req.params.gameId,
      req.session.passport.user
    );
    return res.send({
      success: true,
      status: 'ok',
      message: updatedProp.location.name + ' gespeichert',
    });
  } catch (err) {
 ...
 */
router.post('/dataChanged/:gameId', async function (req, res) {
  if (!req.body.authToken || req.body.authToken !== req.session.authToken) {
    logger.info('Auth token missing, access denied');
    return res
      .status(401)
  ...
  try {
    await gameplays.updateGameplayLastChangedField(
      req.session.passport.user,
      req.params.gameId
    );
    ...
    return res
      .status(500)
      .send({ message: 'Konnte nicht speichern: ' + err.message });
  }
});
...
router.post('/savePositionInPricelist/:gameId', async function (req, res) {
  ...
  try {
    ...
    for (const p of props) {
      await properties.updatePositionInPriceList(
        req.params.gameId,
        p.uuid,
        p.positionInPriceRange
      );
    }
    await gameplays.invalidatePricelist(
      req.params.gameId,
    ...
    return res
      .status(500)
      .send({ message: 'Speichern schlug fehl: ' + err.message });
  }
});
